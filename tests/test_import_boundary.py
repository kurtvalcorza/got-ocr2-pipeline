...
from got_ocr2_pipeline.pipeline import (
    MANIFEST_NAME,
    MODEL_ID,
    MODEL_REVISION,
    GotOcr2Pipeline,
...
_CONFIG = json.dumps({"model_type": "got_ocr2", "image_seq_length": 576}).encode()
...
def test_from_pretrained_refuses_without_snapshot_before_model_imports(tmp_path, forbid_model_imports):
    with pytest.raises(FileNotFoundError, match="allow_download=False"):
        GotOcr2Pipeline.from_pretrained(device="cpu", weights_dir=tmp_path, allow_download=False)
